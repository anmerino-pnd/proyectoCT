...
Permite al agente realizar análisis de datos sobre la red de sucursales ejecutando código Python de manera controlada.

* **Funcionamiento**:

  * Carga un agente de Pandas con información de sucursales.csv.

  * **get_sucursales_info**(): Recibe código Python generado por el LLM, lo ejecuta en un entorno seguro local (exec) y captura la salida estándar (stdout) o el valor de la variable result.

  * **Uso**: Ideal para preguntas complejas como "¿Qué sucursales tienen zona extendida?" o "¿Cuál es la dirección de la sucursal más al norte?", permitiendo al LLM operar los datos matemáticamente en lugar de alucinar.

##### **tools/moneda_api.py**
Herramienta utilitaria para conversión de divisas en tiempo real.

Funcionamiento:

Consulta la tabla monedas_api en MySQL para obtener el tipo de cambio actual.

Realiza la conversión matemática de USD a MXN y devuelve el resultado formateado.
  ...
  * `class Transform`:
    ...
    * `_get_all_specifications() -> dict`: 
      ...
      * **Comportamiento**:
        ...
        4. Transforma y guarda las nuevas fichas en MongoDB (upsert=True).
...
* **Clases y funciones clave**:
  * `class Load`:
    ...
    * `sales_products_vs()`: 
      * **Propósito**: Crea o actualiza el *vector store* para ventas/ofertas.
