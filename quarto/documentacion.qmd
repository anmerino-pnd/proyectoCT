...
3. Asegurarse de estar corriendo Ollama en el ambiente:
   Verifica que el servicio de Ollama esté activo y que el modelo `gemma3:12b` esté disponible.
```bash
ollama serve
...
4. Configurar variables de entorno:
   Antes de levantar el *backend*, asegurarse de que el archivo `.env` en la raíz del proyecto contenga las siguientes variables con sus valores correctos. **Es fundamental no exponera esta información directamente en el código fuente**
```python
...
* Extracción:
  ...
  * La información detallada de las fichas técnicas se extrae de un **servicio externo** a través de solicitudes HTTP/HTTPS, que devuelve los datos en formato XML. Se ha implementado un mecanismo de reintentos y manejo de errores (ej. 403 Forbidden por cambio de IP) para extracción.
...
5. Actualmente se registra el error de IP bloqueada. Sería útil automatizar una notificación. 
...
6. Todas las interacciones (consultas y respustas) se registran en las colecciones `sessions` y `message_backup` en **MongoDB**.
