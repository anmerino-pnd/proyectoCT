...
```
  ...
  En sistemas **Linux**, esto se puede realizar fácilmente mediante un **cron job** y el archivo `reload_products.sh` que:
...
chmod +x ~/proyectoCT/reload_products.sh

# Probar manualmente 
~/proyectoCT/reload_products.sh
...
30 8-18 * * 1-6 ~/proyectoCT/reload_products.sh >> ~/proyectoCT/logs/reload_cron_wrapper.log 2>&1
...
# Cada 1° y 2° de cada mes se ejecuta la tarea
0 9 1,2 * * ~/proyectoCT/reload_sales.sh

crontab -l

cat ~/proyectoCT/logs/monthly_sales_etl.log
```
...
# A partir del 2 de cada mes hasta que acabe el mes, agrega promociones que se hayan subido
