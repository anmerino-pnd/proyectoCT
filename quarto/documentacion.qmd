...
3. Asegurarse de estar corriendo los programas necesarios en el ambiente:
...
# 3. Copiar el archivo JSON al contenedor
...
Los logs también se puede analizar 
```bash
...
En sistemas **Linux**, esto se puede realizar fácilmente mediante un **cron job**.

```bash
# Crea la carpeta de logs
mkdir -p ~/proyectoCT/logs

# Agregar la tarea al cron
crontab -e

# Añade la siguiente línea
30 8-18 * * * ~/proyectoCT/.venv/bin/python3 ~/proyectoCT/src/ct/ETL/update_vector_stores.py >> ~/proyectoCT/logs/update_vector_stores.log 2>&1

# Verificar que se hizo correctamente con
crontab -l

# Cuando ya se haya ejecutado el flujo del cron, se puede revisar con
cat ~/proyectoCT/logs/update_vector_stores.log
```
