...
* **Permisos `sudo`**: El usuario que ejecuta el servicio (`angel.merino`) requiere `sudo` **sin password** para `systemctl restart chatbot-api` y operaciones de Podman desde un cron job. Esto se configura con un archivo en `/etc/sudoers.d/chatbot-api` que delimita el permiso únicamente a los comandos necesarios (ver sección 6.7 para los detalles). Sin este permiso, el flujo de despliegue continuo (CD) automatizado no puede reiniciar la API.
...
* **Clases y funciones clave**:
  ...
  * `class ToolAgent`:
    * `__init__`: 
      ...
      * **Comportamiento**:
        * Establece `self.model` a "gpt-4.1" (O cualquier modelo disponible en [https://reference.langchain.com/python/langchain/models/](https://reference.langchain.com/python/langchain/models/)).
        * Inicializa las conexiones a las colecciones de MongoDB (`sessions`, `message_backup`).
        * Define `self.tools` como una lista de objetos `Tool`, que el agente puede invocar. Estas herramientas incluyen `search_information_tool`, `inventory_tool` y `sales_rules_tool`, entre otras. Cada una con su descripción y esquema de argumentos (`args_schema`) cuando aplica.
    ...
    * `run()`:
      ...
      * **Comportamiento**: 
        ...
        * Define los `inputs` para el `graph`, incluyendo la `query`, `chat_history`, `listaPrecio` y `session_id`.
        * Utiliza `self.graph.ainvoke()` para obtener la respuesta final en modo asíncrono y mandarla al usuario.
...
└── summary_.json        # Resumen del batch con promedios
```

Cuando se habiliten los permisos en MongoDB, el sistema intentará guardar automáticamente en la base de datos — el JSON funciona como respaldo permanente en cualquier caso.
...
mongodb.evaluation_results  # (opcional) Si MongoDB tiene permisos
```

---

### 5.9. Dashboard de monitoreo en tiempo real
...
El CI termina entregando una imagen en `ghcr.io`. Falta el último tramo: que el servidor productivo descargue esa imagen y reinicie el servicio. La opción recomendada es un **cron job local en el servidor** que pollee el registry y reinicie `chatbot-api` cuando detecte una imagen nueva. Es la opción más simple, no requiere abrir puertos ni gestionar llaves SSH, y deja el control en el servidor.

#### 6.7.1. Flujo general

```mermaid
flowchart LR
    A[Developer push a main] --> B[GitHub Actions]
    B --> C{Tests pasan?}
    C -->|No| X[CI rojo, no se publica]
    C -->|Sí| D[buildah-build]
    D --> E[push a ghcr.io]
    E --> F[(ghcr.io/proyecto-ct:latest)]
    F -.poll cada 5 min.-> G[cron en servidor]
    G --> H{digest cambió?}
    H -->|No| I[no-op]
    H -->|Sí| J[sudo systemctl restart chatbot-api]
    J --> K[systemd levanta contenedor con imagen nueva]
```

#### 6.7.2. Paso 1 — Autenticar el servidor contra ghcr.io

En GitHub, generar un **Personal Access Token (classic)** con scope `read:packages`. En el servidor, autenticar Podman una sola vez:

```bash
echo "$GHCR_PAT" | podman login ghcr.io -u <usuario_github> --password-stdin
```

Las credenciales quedan persistidas en `~/.config/containers/auth.json` y el cron las reutiliza sin volver a pedir password.

#### 6.7.3. Paso 2 — Migrar el servicio systemd a Podman (recomendado)

Hasta ahora `chatbot-api.service` ejecuta Gunicorn directo sobre el virtualenv del proyecto (sección 1.4.5.3). Para que `systemctl restart` levante la imagen recién pulleada, conviene cambiar el `ExecStart` a un `podman run`:

```ini
[Service]
ExecStartPre=-/usr/bin/podman stop -t 30 chatbot-api
ExecStartPre=-/usr/bin/podman rm chatbot-api
ExecStart=/usr/bin/podman run --rm --name chatbot-api \
  -p 8000:8000 \
  --env-file /home/angel.merino/proyectoCT/.env \
  -v /home/angel.merino/proyectoCT/static/ssl:/app/static/ssl:ro \
  ghcr.io/anmerino-pnd/proyecto-ct:latest
ExecStop=/usr/bin/podman stop -t 30 chatbot-api
Restart=always
RestartSec=5
```
...
- Los `ExecStartPre` con `-` ignoran fallos: si el contenedor no existía, no aborta el arranque.
- El volumen de `static/ssl` se monta como solo lectura porque la imagen no incluye los certificados — siguen viviendo en el host.
- `--env-file` apunta al `.env` del host; **no se debe** copiar el `.env` dentro de la imagen.

Después de editar el archivo `.service`:
...
#### 6.7.4. Paso 3 — Script de actualización

Crear `~/proyectoCT/update_image.sh`:

```bash
#!/bin/bash
set -euo pipefail

IMAGE="ghcr.io/anmerino-pnd/proyecto-ct:latest"
LOG=~/proyectoCT/logs/update_image.log
mkdir -p "$(dirname "$LOG")"

# Digest actual antes del pull (vacío si no existe localmente)
CURRENT=$(podman image inspect "$IMAGE" --format '{{.Digest}}' 2>/dev/null || echo "none")

# Pull silencioso (logs van al archivo)
podman pull "$IMAGE" >>"$LOG" 2>&1

# Digest después del pull
NEW=$(podman image inspect "$IMAGE" --format '{{.Digest}}')

if [ "$CURRENT" != "$NEW" ]; then
    echo "$(date -Is) imagen actualizada ($CURRENT → $NEW), reiniciando servicio" >>"$LOG"
    sudo /bin/systemctl restart chatbot-api
    echo "$(date -Is) restart completado" >>"$LOG"
else
    echo "$(date -Is) sin cambios" >>"$LOG"
fi
```
...
```bash
chmod +x ~/proyectoCT/update_image.sh
```
...
Añadir, por ejemplo, una corrida cada 5 minutos (ajustable):

```cron
# Polling de la imagen del chatbot cada 5 minutos
*/5 * * * * ~/proyectoCT/update_image.sh
```
...
```bash
crontab -l
tail -f ~/proyectoCT/logs/update_image.log
```

#### 6.7.6. Paso 5 — Permisos `sudo` sin password

El script invoca `sudo systemctl restart chatbot-api` y necesita poder hacerlo sin teclear la contraseña (cron es no-interactivo). Crear un archivo de sudoers acotado:

```bash
sudo visudo -f /etc/sudoers.d/chatbot-api
```

Contenido:

```text
# Permite al usuario de despliegue reiniciar solo el servicio del chatbot
angel.merino ALL=(ALL) NOPASSWD: /bin/systemctl restart chatbot-api
```

Importante:

- El permiso queda **delimitado al comando exacto**; el usuario NO obtiene `sudo` global.
- Usar siempre `visudo` para que valide la sintaxis antes de guardar — un error tipográfico aquí puede bloquear `sudo` para todo el sistema.
- Validar:

```bash
sudo -l -U angel.merino | grep chatbot-api
```

Debe listar exactamente la línea `NOPASSWD: /bin/systemctl restart chatbot-api`.

#### 6.7.7. Limitaciones y consideraciones

- **Latencia**: hasta 5 minutos entre el push y el deploy efectivo (ajustable bajando el intervalo del cron, pero más frecuente = más llamadas a `ghcr.io`).
- **Sin rollback automático**: si la imagen nueva está rota, `systemd` la reinicia indefinidamente. Mitigación: mantener un tag `:stable` además de `:latest` y promover manualmente, o agregar una verificación de health-check antes de marcar el deploy como exitoso.
- **Sin alertas de pull fallido**: si `podman pull` falla por red, el cron reintenta en el siguiente tick. Considerar añadir notificación (Slack/email) si el log muestra errores recurrentes.
- **Visibilidad de logs**: los logs del contenedor van a `journalctl -u chatbot-api` (como antes); los logs del cron de despliegue van a `~/proyectoCT/logs/update_image.log`.

#### 6.7.8. Alternativas consideradas y descartadas

- **SSH desde GitHub Actions** (`appleboy/ssh-action`): requiere abrir SSH a internet o mantener una IP allowlist, gestionar una llave privada como secret de GitHub, y añade un punto de falla externo al servidor. El cron es más simple y deja al servidor en control de cuándo se actualiza.
- **Webhook `repository_dispatch`**: requiere correr un listener HTTP en el servidor (Flask, GitHub Webhook receiver), abrir un puerto público, validar firmas HMAC. Más infraestructura para mantener; solo justificable si se necesita deploy inmediato (latencia menor a 1 minuto).
- **Watchtower / Podman auto-update**: existen daemons que monitorean cambios de imagen automáticamente. Son válidos pero introducen una dependencia más; el cron de 4 líneas cumple la misma función con código que cualquiera del equipo puede leer.
