...
Para que `pytest` pueda correr sin credenciales reales (sin MongoDB, sin OpenAI, sin archivos FAISS), tres módulos del proyecto inicializan recursos externos de forma **diferida** (*lazy*):

* **`src/ct/settings/clients.py`** expone `get_mongo_client()` y `get_db()` decoradas con `functools.lru_cache`. La conexión a MongoDB se crea la primera vez que se solicita, no al importar el módulo.

* **`src/ct/tools/search_information.py`** define `reload_vector_store()` pero **no la invoca al importarse**. La carga del vector store FAISS ocurre en producción mediante el evento `lifespan` de FastAPI definido en `src/ct/main.py`, que se ejecuta al arrancar el servidor (Gunicorn/Uvicorn), no durante los tests.

* **`src/ct/chat.py`** expone `get_assistant()` con `lru_cache` en lugar de instanciar `ModeratedToolAgent()` a nivel de módulo. Esto evita que el agente intente conectarse a Mongo o cargar modelos LLM cuando un test simplemente importa `ct.main`.

Este patrón es importante: si en el futuro alguien añade un nuevo módulo que requiere recursos externos, debe seguir el mismo esquema (factory + `lru_cache`) para no romper el pipeline de CI.
...
### 6.6. Publicación de la imagen en GitHub Container Registry (opcional)

Cuando se desee publicar automáticamente la imagen, descomentar el último bloque de `.github/workflows/ci.yml`:
...
No requiere configurar secretos adicionales: GitHub Actions provee `GITHUB_TOKEN` automáticamente, y solo hace falta otorgar permisos de escritura al *workflow* desde la configuración del repositorio (Settings → Actions → General → Workflow permissions → Read and write).

### 6.7. Solución de problemas comunes
